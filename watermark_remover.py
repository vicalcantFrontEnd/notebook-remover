...
REMOVAL_METHODS = ["pixel", "box", "crop"]
...
def process_video(
    ...
    mask = None
    if method == "pixel":
        # Multi-frame consensus mask: sample several frames to build a
        # more accurate and stable watermark mask.
        masks = []
        sample_times = [effective_duration * (i + 1) / 6 for i in range(5)]
        for ts in sample_times:
            cmd = [
                FFMPEG_BIN, "-ss", str(ts), "-i", str(input_path),
                "-vframes", "1", "-f", "rawvideo", "-pix_fmt", "bgr24",
                "-v", "quiet", "-",
            ]
            res = subprocess.run(cmd, capture_output=True, creationflags=nw)
            if res.returncode == 0 and len(res.stdout) >= w * h * 3:
                sample = np.frombuffer(res.stdout, dtype=np.uint8).reshape((h, w, 3)).copy()
                masks.append(create_pixel_mask(sample, region, sensitivity=sensitivity))
        if masks:
            # Consensus: pixel is watermark if detected in >=60% of samples
            ...
            first_frame, _ = extract_first_frame(input_path)
            mask = create_pixel_mask(first_frame, region, sensitivity=sensitivity)
